...
pub const SPLATTER_ALT_INTENSITY_SCALE: f32 = 5.0;

pub const SPLATTER_ALT_MASK_SCALE: f32 = 4.0;

pub fn splatter_alt_scale(intensity: f32, mask_r: f32) -> f32 {
...
pub fn splatter_alt_texel_straight(color_rgba: [u8; 4], mask_r: u8, intensity: f32) -> [u8; 4] {
    let premul = splatter_alt_texel(color_rgba, mask_r, intensity);
    let a = premul[3];
    if a == 0 {
        return [0, 0, 0, 0];
    }
    let af = a as f32;
    [
        ((premul[0] as f32 / af) * 255.0).min(255.0) as u8,
        ((premul[1] as f32 / af) * 255.0).min(255.0) as u8,
        ((premul[2] as f32 / af) * 255.0).min(255.0) as u8,
        a,
    ]
}
