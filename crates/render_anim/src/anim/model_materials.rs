//! Lit model-pass materials for every body, world-weapon and map-model surface,
//! admitted once per material generation before the match is Ready, and the
//! bind-pose DObj of every map model. Remote bodies, dropped items, script
//! models and dynamic entities look them up here; none admits a material or
//! composes a single-model DObj while drawing.
...
/// Material catalog, lighting atlas, body catalog (by address) and world
/// weapon catalog identity. A world without an atlas or without catalogs still
/// settles, with nothing admitted.
...
impl PreparedModelMaterials {
    fn owns(&self, owner: &ModelMaterialsOwner) -> bool {
        self.owner.as_ref().is_some_and(|(catalog, atlas, bodies, world)| {
    ...
    /// The table binds to this material catalog. The loading screen holds
    /// until it does.
    ...
    /// The admitted material for a surface's present name, when the table
    /// belongs to `catalog`. `None` for a name the session refused.
    pub fn material(
        &self,
    ...
    /// The admitted material for a map model surface's authored material.
    pub fn authored(
        &self,
        catalog: &Arc<RuntimeMaterialCatalog>,
    ...
    /// The bind-pose DObj of a single map model.
    pub fn scene_dobj(&self, model: &str) -> Option<&Arc<assets::DObj>> {
        self.scene_dobjs.get(model)
    }

    /// The admitted material for a projectile surface's authored material.
    ...
    /// The bind-pose DObj of a projectile model.
...
/// A map model's lit pass material for one authored material: its own maps,
/// state and sort key, and the session's model lighting atlas.
...
pub fn prepare_model_materials(
    ...
    let owner: ModelMaterialsOwner = (
        ...
        bodies.as_ref().map(|bodies| Arc::as_ptr(&bodies.0) as usize),
    ...
    diag::info!(
        ...
        if refused.is_empty() {
            ...
            format!(
                " first_refused=[{}]",
                refused.iter().take(8).cloned().collect::<Vec<_>>().join(",")
            )
        }
    );
    *prepared = PreparedModelMaterials {
