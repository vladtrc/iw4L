//! First-person weapons as the session prepared them.
//!
//! Before the match is Ready every weapon that has a first-person model gets
//! its materials admitted, its models laid out and its compositions bound to
//! clips, per kit side. The FPV consumer reads this table and nothing wider:
//! equipping, picking up, switching back and toggling the rocket choose a
//! prepared rig. Device products stay the renderer's — images and the lighting
//! atlas are handles it already owns.
...
pub struct FpvPreparationCensus {
    ...
    pub rigs: usize,
    pub materials: usize,
    pub ready_sides: usize,
    pub refused_sides: usize,
    pub elapsed_ms: f64,
}

/// Everything the first-person consumer reads about weapons, keyed by weapon
/// id. Its material bindings belong to one material catalog and one
/// first-person mesh catalog; a table for another pair is another table.
...
impl FpvWeaponTable {
    ...
    pub fn slot(&self, weapon: u32, axis: bool) -> &FpvWeaponSlot {
        self.slots
            .get(weapon as usize)
...
impl PreparedFpv {
    ...
    /// The table exists and binds to this material catalog. The loading screen
    /// holds until this is true.
...
impl FpvPreparationJob {
    fn new(
        ...
        for id in 1..=weapon_n {
            for axis in [false, true] {
                let Some(sides) = registry.fpv_assemblies_of(id, axis) else {
                    continue;
                };
        ...
        let work_total = (models.len() + layouts_queue.len()) as u64 + u64::from(weapon_n);
            ...
            match verdict {
                ...
                FpvSurfaceVerdict::Inapplicable(NO_COLOUR_MAP) => {
                    ...
                    if let Some(name) = name
                        ...
                        && !census.gun_colormap_skip_names.iter().any(|seen| seen == name)
                    {
    ...
    fn prepare_next_weapon(&mut self, fpv: &FpvMeshCatalog, xanims: &XAnimCatalog) {
        ...
        self.guns.push(gun);
        if id == 0 {
            self.slots.push([FpvWeaponSlot::Absent, FpvWeaponSlot::Absent]);
            return;
        }
        ...
        let sides: [FpvWeaponSlot; 2] = [false, true].map(|axis| {
            let (Some((hands, hands_index)), Some(assemblies)) = (
                ...
                registry.fpv_assemblies_of(id, axis),
            ) else {
...
/// A table whose every weapon is refused for one reason the whole session
/// shares, such as a world without model lighting. The loading screen does not
/// wait on it.
fn refused_table(
    owner: &FpvPreparationOwner,
    fpv: &FpvMeshCatalog,
...
/// Build the first-person table for the installed match, a budgeted slice per
/// frame. It starts once the material generation is admitted and its images
/// are installed, and it is replaced whenever any of what it binds to is.
pub fn prepare_fpv_compositions(inputs: PrepareFpvInputs, mut prepared: ResMut<PreparedFpv>) {
    ...
    let restart = prepared.job.as_ref().is_none_or(|job| !job.owner.same(&owner));
    if restart {
        prepared.clear();
