...
impl AnimDObjSceneSkels {
    /// Put every soldier kit model and world weapon model in before anything
    /// is drawn, so no first sighting copies a skeleton.
    ...
    /// A model whose skeleton is already shared: it is taken, never copied.
    pub fn shared(
        &mut self,
        name: &str,
