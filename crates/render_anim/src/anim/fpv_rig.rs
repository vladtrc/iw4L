...
//!
//! Everything here is settled before the match is Ready: which surfaces of each
//! model draw, with which admitted material, in which vertex layout, and how
//! every clip's tracks land on the combined skeleton. Equipping a weapon picks
//! a rig; it builds nothing. Both hands of a dual-wield composition are slots
//! in one layout, so the left gun is a second destination range over the same
//! prepared model.
...
/// What the session decided about one surface's material, once, for every
...
/// The materials every first-person model of the session draws with, and the
/// verdict on every surface that asked for one.
#[derive(Default)]
...
impl FpvMaterialAdmission {
    ...
    pub fn is_recorded(&self, catalog_entry: usize) -> bool {
        self.verdicts.contains_key(&catalog_entry)
    }
...
/// One model with one set of hide words, laid out once for every composition
/// that mounts it.
...
/// The first surface a model needs and cannot draw. It names the model, the
/// surface and the material, so the refusal is addressable before anyone
/// equips the weapon.
#[derive(Clone, Debug, PartialEq, Eq)]
...
impl PreparedFpvComposition {
    /// `model_of` hands out the prepared model for one catalog entry under one
    /// set of hide words.
    pub fn compose(
        assembly: Arc<FpvAssembly>,
...
/// Tracks of one clip on one assembly's combined skeleton, as bone indices.
/// `FpvClipTracks::NONE` is a track that drives no bone.
...
/// The rows the plan publishes for as long as the composition holds. The index
/// list is not stored per rig: it is the prepared models' own lists, rebased
/// into each slot's destination range when the plan installs it.
...
impl PreparedFpvRig {
    ...
    /// `tracks` holds, per hand, the composed tracks of every clip slot the
    /// weapon plays in that hand.
    pub fn build(
        composition: Arc<PreparedFpvComposition>,
        dual: bool,
        admission: &FpvMaterialAdmission,
