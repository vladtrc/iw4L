...
fn skip_rewrite(latch: &FlashWhiteoutLatch, alpha: f32, win_w: f32, win_h: f32) -> bool {
...
pub(crate) fn update_flash_whiteout(
    ...
    *job = FlashGpuJob::Idle;
    ...
    if skip_rewrite(&latch, white, win_w, win_h) {
    ...
    latch.packed = crate::gpu_list::pack_modulate(&quad, color);
    latch.last_w = Some(win_w);
    latch.last_h = Some(win_h);
    latch.last_alpha = Some(white);
    *job = FlashGpuJob::Write;
}
