...
/// The flash overlay: the scene saved when the flash began, fading on the
/// shot curve, under white fading on the white curve.
...
pub(crate) struct FlashWhiteoutLatch {
    ...
    last_w: Option<f32>,
    last_h: Option<f32>,
    /// The shellshock start the saved screen belongs to. A new start is a new
    /// flash and takes a new screen.
