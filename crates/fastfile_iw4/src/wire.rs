...
impl WirePointer {
    pub fn decode(raw: u64, format: Iw4WireFormat) -> Option<Self> {
        let following = match format {
            Iw4WireFormat::X86 => u64::from(u32::MAX),
            Iw4WireFormat::X64 => u64::MAX,
        };
        if raw == 0 {
            Some(Self::Null)
...
impl WireAssetTable<'_> {
    pub fn format(&self) -> Iw4WireFormat {
        self.format
    }

    pub fn script_strings(&self) -> impl Iterator<Item = &[u8]> + '_ {
        let mut inline = 0;
        self.string_slots
            .chunks_exact(self.format.pointer_bytes())
            .map(move |slot| {
                let start = match pointer(slot, 0, self.format).unwrap() {
                    WirePointer::Null => return &[][..],
                    WirePointer::Following => inline,
                    WirePointer::Offset(p) => p.offset as usize - self.string_slots.len(),
                    WirePointer::Insert => unreachable!("validated script-string pointer"),
                };
                let len = self.string_data[start..]
                    .iter()
                    .position(|b| *b == 0)
                    .unwrap();
                if matches!(
                    pointer(slot, 0, self.format).unwrap(),
                    WirePointer::Following
                ) {
                    inline = start + len + 1;
                }
                &self.string_data[start..start + len]
            })
    }
...
fn pointer(data: &[u8], at: usize, format: Iw4WireFormat) -> Result<WirePointer, WireTableError> {
    let mut raw = [0; 8];
...
pub fn read_wire_asset_table(
    ...
    for (at, count) in [
        ...
        match (count, pointer(data, at, format)?) {
            (0, WirePointer::Null) | (1.., WirePointer::Following) => {}
    ...
    let string_data = &data[slots + strings_size..cursor];
    ...
    Ok(WireAssetTable {
        ...
        entries,
        string_slots: &data[slots..slots + strings_size],
        string_data,
    })
}
